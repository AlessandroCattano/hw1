...
function AddToList(){
    const url = `Add.php?id=${encodeURIComponent(single_id)}`;
...
function addSection(json){
    ...
    if(json[0].error){
        ...
        paragraph.textContent = json[0].zero;
...
function onJsonProvider(json){
    ...
    if(json.length == 0){
        const urlp = `AddToDb.php?id=${encodeURIComponent(single_id)}`;
        fetch(urlp).then(onResponse).then(printProviders);
    }else{
        printProviders(json);
    }

    const url_track = `getTrack.php?title=${encodeURIComponent(title)}`;
...
function onJsonSpotify(json) {
    ...
    const url_reviews = `getReviews.php?id=${encodeURIComponent(single_id)}`
...
function onJson(json) {
    ...
    const url = `Check.php?id=${encodeURIComponent(single_id)}`
    ...
    const url_provider = `getProviders.php?id=${encodeURIComponent(single_id)}`;
...
function getdetails(clickedPoster) {
    const imdbId = clickedPoster.id;
    console.log("getdetails: singolo imdb da caricare: " + imdbId);
    fetch('omdb.php', {
        method: 'POST',
        headers: {
...
function searchContent(){
    ...
    if(!trimmedValue){
        ...
        const url = `Search.php?title=${encodeURIComponent(value)}`;
